        ...
        <div class="pt-5">
            <div class="panel">
                <div class="flex items-center justify-between mb-5">
                    <h5 class="font-semibold text-lg dark:text-white-light">{{ $kullanici->ad_soyad }} Kullanıcısını Düzenle</h5>
                </div>
                ...
                <form class="space-y-5" method="POST" action="{{ route('kullanicilar.update', $kullanici->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" class="form-input" value="{{ old('email', $kullanici->email) }}" required />
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <select id="rol_id" name="rol_id" class="form-select" required>
                                ...
                                @foreach($roller as $rol)
                                    <option value="{{ $rol->id }}" {{ old('rol_id', $kullanici->rol_id) == $rol->id ? 'selected' : '' }}>
                                        {{ $rol->rol_adi }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input id="telefon" name="telefon" type="text" class="form-input" value="{{ old('telefon', $kullanici->telefon) }}" />
                        </div>
                    </div>
                    <div>
                        ...
                        @if($kullanici->resim)
                            <div class="flex items-center mb-3">
                                <img src="{{ asset('storage/kullanici-resimleri/'.$kullanici->resim) }}" alt="avatar" class="w-20 h-20 rounded-full object-cover" />
                                <span class="ltr:ml-2 rtl:mr-2">Mevcut Resim</span>
                            </div>
                        @endif
                    ...
                    <div>
                        <label class="inline-flex cursor-pointer">
                            <input type="checkbox" name="aktif" class="form-checkbox" value="1" {{ old('aktif', $kullanici->aktif) ? 'checked' : '' }} />
                            <span class="text-white-dark">Aktif</span>
                        </label>
                    </div>
                    <button type="submit" class="btn btn-primary !mt-6">Güncelle</button>
                </form>
            </div>
        </div>
    </div>
